                    ...
                    if (sequenceNumber < sequenceCount) {
                        windowMutex.lock();
                         ...
                         if (!windowMaskSending[i]) { 
                            ...
                            //implementation of simulated losses 
                            if(drops[drop_seq] == sequenceNumber){
                                //determines if the packet will be "lost" or ack "lost"
                                if(strcmp(error, "d")){
                                    sendto(socketBucket, frame, frameLength, 1, (const struct sockaddr *) &serverAddress,
                                        sizeof(serverAddress));//drop packets
                                }else{
                                    sendto(socketBucket, frame, frameLength, 2, (const struct sockaddr *) &serverAddress,
                                        sizeof(serverAddress));//lost ack
                                }
                                drop_seq = drop_seq +1;
